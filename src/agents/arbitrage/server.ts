...
if (!agentPrivateKey) {
  console.error('AGENT_ARBITRAGE_PRIVATE_KEY not set')
  process.exit(1)
}
...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    const analysis = await analyzeArbitrage(question, x402Fetch)
    return res.json(analysis)
  } catch (err) {
    console.error('[arbitrage] analyze error:', err)
    return res.status(500).json({ error: 'Internal error', details: String(err) })
  }
})

app.get('/health', (_req, res) => res.json({ agent: 'arbitrage', status: 'ok', port: PORT }))

app.listen(PORT, () => {
  console.log(`[arbitrage] Agent server listening on port ${PORT} (price: ${PRICE} USDC)`)
})
