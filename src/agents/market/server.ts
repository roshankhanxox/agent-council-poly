...
if (!agentPrivateKey) {
  console.error('AGENT_MARKET_PRIVATE_KEY not set')
  process.exit(1)
}
...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    const analysis = await analyzeMarket(question, marketUrl, x402Fetch)
...
app.get('/health', (_req, res) => res.json({ agent: 'market', status: 'ok', port: PORT }))

app.listen(PORT, () => {
  console.log(`[market] Agent server listening on port ${PORT} (price: ${PRICE} USDC)`)
})
